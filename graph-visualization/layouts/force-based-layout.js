...
Layout.ForceDirected = function(graph, options) {
  ...
  var calcPositions = function(graph, node) {
    ...
    var new_disp_x = node.layout.offset_x / delta_length * Math.min(delta_length, temperature);
    var new_disp_y = node.layout.offset_y / delta_length * Math.min(delta_length, temperature);

    var c = 50;
    node.position.x += Math.max(-c, Math.min(c,new_disp_x));
    node.position.y += Math.max(-c, Math.min(c,new_disp_y));
  };
